...
def follow_wrist_traj(
    ...
    required_indices = [wrist_index, pinch_a_index, pinch_b_index]
    ...
    targets = offsets
    executed = 0
    target_quat = transform_utils.mat2quat(base_rot)
    first_move = True
    first_move_dt = 0.02
    for idx, (target_pos, gripper_action) in enumerate(
        zip(targets, grip_actions)
    ):
    ...
    logger.info(f"Executed frames: {executed}/{len(targets)}")
