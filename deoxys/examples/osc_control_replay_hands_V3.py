...
def follow_wrist_traj(
    ...
    required_indices = [pinch_a_index, pinch_b_index]
    ...
    for idx, (target_center, gripper_action, hand_dir) in enumerate(
        ...
        if not np.isfinite(target_center).all():
            continue
        if not np.isfinite(hand_dir).all():
            continue
        ...
        align_quat = _quat_from_two_vectors(current_y, desired_y)
        target_quat = transform_utils.quat_multiply(align_quat, current_quat)
        if np.dot(target_quat, current_quat) < 0.0:
            target_quat = -target_quat

        target_rot = transform_utils.quat2mat(target_quat)
