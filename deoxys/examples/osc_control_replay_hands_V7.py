...
def _get_gripper_width(robot_interface):
    ...
    if width is None:
        return None
    return float(np.array(width).reshape(-1)[0])


def _get_gripper_state_info(robot_interface):
    if robot_interface.gripper_state_buffer_size == 0:
        return None
    state = robot_interface._gripper_state_buffer[-1]
    return {
        "is_grasped": bool(state.is_grasped),
        "max_width": float(state.max_width),
        "temperature": int(state.temperature),
        "time_sec": float(state.time.toSec),
        "time_msec": int(state.time.toMSec),
    }
...
def _format_state(
    ...
    parts = []
    if pinch_width is None or not np.isfinite(pinch_width):
        parts.append("pinch=nan")
    else:
        parts.append(f"pinch={pinch_width:.4f}")
    if current_width is None or not np.isfinite(current_width):
        parts.append("width=nan")
    else:
        parts.append(f"width={current_width:.4f}")
    if dwidth is None or not np.isfinite(dwidth):
        parts.append("dwidth=nan")
    else:
        parts.append(f"dwidth={dwidth:.4f}")
    if gripper_state is None:
        parts.append("grasped=nan")
    else:
        parts.append(f"grasped={int(gripper_state['is_grasped'])}")
        parts.append(f"maxw={gripper_state['max_width']:.4f}")
        parts.append(f"temp={gripper_state['temperature']}")
        parts.append(
            f"t={gripper_state['time_sec']:.3f}s:{gripper_state['time_msec']}ms"
        )
    parts.append(f"closed={int(gripper_closed)}")
    if current_pos is not None:
        ee_z = float(np.array(current_pos).reshape(3)[2])
        parts.append(f"ee_z={ee_z:.4f}")
...
def follow_wrist_traj(
    ...
    thumb_mcp = hand_joints[:, 2, :]
    index_pip = hand_joints[:, 6, :]
    index_root = hand_joints[:, index_root_index, :]
    pinch_dist = np.linalg.norm(pinch_a - pinch_b, axis=1)
    max_width = 0.08
    force_close_threshold = 0.06
    force_open_threshold = 0.07
    grasp_force = 15.0
    required_indices = [
        wrist_index,
        pinch_a_index,
    ...
    hand_centers = (pinch_a + pinch_b + thumb_mcp + index_pip) * 0.25
    ...
    for idx, (target_center, hand_dir) in enumerate(zip(hand_centers, hand_dirs)):
        ...
        if (
            ...
            continue
        pinch_width = pinch_dist[idx]
        current_width = _get_gripper_width(robot_interface)
        gripper_state = _get_gripper_state_info(robot_interface)
        if current_width is not None and last_width is not None:
            dwidth = current_width - last_width
        else:
            dwidth = None
        ...
        current_rot = current_pose[:3, :3]
        current_quat = transform_utils.mat2quat(current_rot)
        current_ee_z = float(np.array(current_pos).reshape(3)[2])
        if last_ee_z is None:
            dz = None
        else:
            dz = current_ee_z - last_ee_z
        last_ee_z = current_ee_z
        state_str = _format_state(
            pinch_width,
            current_width,
            dwidth,
            gripper_state,
        ...
        if np.isfinite(pinch_width):
            if gripper_closed and pinch_width >= force_open_threshold:
                gripper_closed = False
                state_str = _format_state(
                    pinch_width,
                    current_width,
                ...
                _log_event(
                    log_f,
                    f"{time.time():.3f} state_open idx={idx} {state_str}",
                )
            elif not gripper_closed and pinch_width < force_close_threshold:
                gripper_closed = True
                state_str = _format_state(
                    pinch_width,
                    current_width,
                ...
                _log_event(
                    log_f,
                    f"{time.time():.3f} state_close idx={idx} {state_str}",
                )
        ...
        if np.isfinite(pinch_width):
            if gripper_closed:
                send_gripper_grasp(robot_interface, force=grasp_force)
                _log_event(
                    log_f,
                    f"{time.time():.3f} grasp idx={idx} {state_str}",
                )
            elif pinch_width >= force_open_threshold:
                send_gripper_move(
                    robot_interface, width=min(pinch_width, max_width)
                )
                _log_event(
                    log_f,
                    f"{time.time():.3f} move idx={idx} {state_str}",
                )
        gripper_action = 0.0
...
def main():
    ...
    robot_interface.has_gripper = True
    reset_joints_to(
        robot_interface, reset_joint_positions, gripper_open=True
    )
    robot_interface.has_gripper = False
    ...
    log_path = os.path.join(
        log_dir, f"osc_control_replay_hands_V7_{time.strftime('%Y%m%d_%H%M%S')}.log"
    )
    with open(log_path, "w", encoding="utf-8") as log_f:
        _log_event(
            log_f,
            f"{time.time():.3f} start npz={base_npz_path} hand={args.hand}",
        )
        follow_wrist_traj(
            robot_interface,
            controller_type,
        ...
        _log_event(log_f, f"{time.time():.3f} end")
